<template>
  <div>
    ...
    <AdminEditorLayout
      :count-label="`${items.length} mini puzzle${items.length === 1 ? '' : 's'}`"
      create-label="Add mini"
      empty-label="No mini crosswords yet."
      :loading="loading"
      :empty="!items.length"
      @create="openNew"
    >
      ...
      <template #form>
        <form v-if="showForm" class="space-y-4" @submit.prevent="save">
          ...
          <p class="text-xs text-[hsl(var(--muted-foreground))]">Optional R/C = fixed start row/col for a compact mini grid.</p>
        ...
        <p v-else class="text-sm text-[hsl(var(--muted-foreground))]">Select a mini to edit, or add a new one.</p>
      </template>
    </AdminEditorLayout>
  </div>
</template>
...
async function onDelete(id: string) {
  if (!confirm('Delete this mini crossword?')) return
  await remove(id)
...
useHead({ title: 'Mini Crossword · Admin' })
</script>
